...
function openEntry(id){
  ...
  $('#readerContent').innerHTML=`<header class="entry-header"><p class="entry-kicker"><time datetime="${esc(e.date||'')}">${esc(dateLabel(e))}</time><span>${esc(e.type)}</span></p><h2 id="readerTitle">${esc(title)}</h2>${e.author?`<p class="byline">${esc(e.author)}</p>`:''}<a class="jump-to-text" href="#articleText">Jump to text <span aria-hidden="true">↓</span></a></header><div class="reading-area"><section class="image-panel" aria-label="Article images"><div class="image-controls"><div><button data-zoom="out" aria-label="Zoom out">−</button><output id="zoomLevel" aria-live="polite">100%</output><button data-zoom="in" aria-label="Zoom in">+</button><button data-zoom="reset">Fit</button></div></div><div class="clipping-pages">${imageHTML}</div></section><section class="text-panel" id="articleText" tabindex="-1" aria-label="Article text">${textHTML}</section></div>`;
